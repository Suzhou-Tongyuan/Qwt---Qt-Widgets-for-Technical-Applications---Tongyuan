...
public:
    ...
    enum LegendLayout
    {
        ...
        //! The legend will be floating 
        FloatLegend,

        //! The legend will be hide
        HideLegend,
    };
    enum FloatPosition
    {
    ...
    QwtText ScaleIndexLabel(int axisId) const;
    void updateScaleIndexText(int axisId);

    void setScaleIndexVisible(int axisId,bool visible);
    bool scaleIndexVisible(int axisId);

    // Legend

    void insertLegend( QwtAbstractLegend *, 
        LegendLayout = QwtPlot::RightLegend, double ratio = -1.0 );
...
Q_SIGNALS:
    ...
    void SignalFixedDimOfAxisChanged(int axis, int dim);

    void SignalHintDimOfAxisChanged(int axis, int dim);

    void SignalLayoutUpdated();
    /*!
...
public Q_SLOTS:
    ...
    void adjustLegendOffset(QPoint global_pos);
protected:
...
private:
    ...
    void setScaleIndexLabel(int axisId, const QwtText &);
