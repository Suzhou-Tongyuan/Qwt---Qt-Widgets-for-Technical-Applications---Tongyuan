...
void QwtLegendLabel::setDown(Qt::MouseButtons mouseButton, bool down )
{
...
void QwtLegendLabel::mousePressEvent( QMouseEvent *e )
{
    if ( e->button() == Qt::LeftButton || e->button() == Qt::RightButton )
    {
        switch ( d_data->itemMode )
        {
            ...
            {
                setDown( e->button(), true );
                return;
            }
            case QwtLegendData::Checkable:
            {
                setDown( e->button(), !isDown() );
                return;
            }
            default:;
        }
...
void QwtLegendLabel::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if ((e->pos() - d_data->lastPos).manhattanLength() > QApplication::startDragDistance())
    {
        if ( d_data->itemMode == QwtLegendData::Clickable )
        {
            if ( d_data->isDown )
    ...
    else if (rect().contains(e->pos()) != d_data->isDown) {
        d_data->isDown = !d_data->isDown;
        update();
        QApplication::flush();
...
void QwtLegendLabel::keyPressEvent( QKeyEvent *e )
{
    if ( e->key() == Qt::Key_Space )
    {
        switch ( d_data->itemMode )
        {
            ...
            {
                if ( !e->isAutoRepeat() )
                    setDown(Qt::NoButton, true );
                return;
            }
            case QwtLegendData::Checkable:
            {
                if ( !e->isAutoRepeat() )
                    setDown(Qt::NoButton, !isDown() );
                return;
            }
            default:;
        }
