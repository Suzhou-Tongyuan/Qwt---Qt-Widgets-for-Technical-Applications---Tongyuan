...
static const double _eps = 1.0e-12;
...
/*!
  \brief 计算给定常量值的包围盒，如果提供了最大刻度和刻度间步长，则计算之

  如果要求计算刻度间步长，则：
      如果 v == 0.0 ，则stepSize = 0.1，否则取value的5位有效数字，并去掉后面的0，得到的数字末尾所在位数，作为stepSize。
      包围盒区间为 [v - maxMajor/2 * stepSize, v + maxMajor/2 * stepSize],
  否则按照简单倍数法计算：
      如果 v == 0.0，包围盒区间为[-0.5, 0.5]
      否则区间为[0.5 * v, 1.5 * v]

  \param value 给定常量值 (In)
  \param maxMajor 最大刻度个数，此参数是可选的，其值<=0 则不需要计算刻度步长 (In)
  \param stepSize 单个刻度步长，此参数是可选，传入空则不需要计算(Out)
  \return 计算出的包围盒区间
*/
QwtInterval QwtScaleEngine::buildInterval( double value, int maxMajor, double *stepSize ) const
{
    double delta;
    maxMajor -= 1;
    if ( maxMajor > 0 && stepSize != nullptr )
    {
        if ( value == 0.0 )
        {
            *stepSize = 0.1;
            delta = 0.1 * maxMajor / 2;
        }
        else
        {
            // 刻度计算思路，取最多5位有效数字，刻度就是最后一位有效数字所在的量级，
            // 假设最大maxMajor == 10，那么：
            //      123456789，刻度大小是10000，表示为12345 X 10000，
            //      1.23456789，刻度大小是0.0001，表示为12345 X 0.0001，
            //      1.20000789，先取出12000，然后消0后为12，刻度大小是0.1，表示为12 X 0.1

            // 获得5位有效数字的整数值
            int value_index = qFloor(log10(fabs( value )));
            double val_base_10000 = value * pow( 10.0, 4.0 - value_index );
            int value_5_precision = qRound( val_base_10000 ) ;

            // 计算末尾0的个数，并得到末尾不带0的整数值
            *stepSize = 1.0 ;
            while ( ( value_5_precision % 10 ) == 0 )
            {
                *stepSize = *stepSize * 10;
                value_5_precision /= 10;
            }

            // 基于5位有效数字的整数值后的指数
            *stepSize = *stepSize * pow( 10.0, value_index - CONSTANT_PRECISION );

            delta = maxMajor/2 * *stepSize;
        }
    }
    else
    {
        delta = ( value == 0.0 ) ? 0.5 : qAbs( 0.5 * value );
    }
...
/*!
  ...
  \param maxNumSteps 允许的最大刻度数
  \param x1 坐标最小值(In/Out)
  \param x2 坐标最大值(In/Out)
  \param stepSize 刻度的长度(Out)
    ...
    double &x1, double &x2, double &stepSize ) const
{
    ...
    //判断以当前的有效数字位数，是否当作直线来显示
    bool is_line = false;
    int precision_threhold = 13;//显示精度以13位有效数字计, 这个数值决定了何时把曲线看成直线
    ...
    {
        //（最大-最小）值之差的量级，跟其任一非0的极值量级比较
        // 如果量级差异超过阈值，则判定为直线（差异忽略不计）
        double max_v = interval.maxValue();
        double min_v = interval.minValue();
        int offset_log10 = qFloor(log10(fabs( max_v - min_v )));
        double base_v = min_v != 0.0 ? min_v : max_v;
        int base_log10 = log10(fabs( base_v ));
        if ( base_log10 - offset_log10 + 1 > precision_threhold ) 
        {
            is_line = true;
        }
    }
    ...
    if ( is_line )
    {
        interval = buildInterval( interval.minValue(), maxNumSteps, &stepSize );
    }
    else
    {
