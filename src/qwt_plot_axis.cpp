...
void QwtPlot::updateAxes()
{
    ...
    for ( int axisId = 0; axisId < axisCnt; axisId++ )
    {
        ...
        if ( !d.isValid )
        {
            int offset_interval_ratio = log10(qMax(fabs(maxValue), fabs(minValue))) - log10(fabs(maxValue - minValue));
            //当基数偏移和刻度的比值大于10的13次方时，禁止继续放大，策略为调整刻度时始终按照原先的刻度最小值和刻度最大值
            //调整，而非按照选中区域的刻度最小值和刻度最大值调整
            //当最大值最小值超过1e307时也禁止继续缩放
            bool enable_rescale = offset_interval_ratio < 13 &&
                abs(minValue) < 1e307 &&
                abs(maxValue) < 1e307 &&
                minValue < maxValue;
            if (enable_rescale)
            ...
            {
                d.scaleDiv = d.scaleEngine->divideScale(
                    d.scaleDiv.lowerBound(), d.scaleDiv.upperBound(),
