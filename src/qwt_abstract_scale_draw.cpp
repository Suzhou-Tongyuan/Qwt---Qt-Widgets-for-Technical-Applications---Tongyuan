    ...
    const QPalette& palette ) const
{
    ...
    if ( hasComponent( QwtAbstractScaleDraw::Labels ) && d_data->labelVisible )
    {
        painter->save();
        ...
        {
            int interval;
            double v1 = majorTicks[0];
            for (interval = 1; interval < majorTicks.count(); interval++)
            {
                double v2 = majorTicks[interval];
                if (!lableOverLap(painter, v1, v2))
                {
                    break;
                }

            }
            for (int i = 0; i < majorTicks.count(); i += interval)
            {
...
bool QwtAbstractScaleDraw::lableOverLap( QPainter*, double /*value1*/, double /*value2*/ ) const
{
    return false;
}

void QwtAbstractScaleDraw::setDim( QRect len )
{
    d_data->dim = len;
}

QRect QwtAbstractScaleDraw::dim() const
{
    return d_data->dim;
}
/*!
