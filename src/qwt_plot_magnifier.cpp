...
void QwtPlotMagnifier::rescale( double factor )
{
    ...
    for ( int axisId = 0; axisId < QwtPlot::axisCnt; axisId++ )
    {
        ...
        {
            QwtScaleMap map = plt->canvasMap( axisId );
            double p1 = map.transform( plt->axisScaleDiv( axisId ).lowerBound() );
            double p2 = map.transform( plt->axisScaleDiv( axisId ).upperBound() );
            double pcenter = ( p1 + p2 ) / 2;
            double pdiff = p1 - pcenter;
            double rescaleDp = pdiff * factor;
            p1 = pcenter + rescaleDp;
            p2 = pcenter - rescaleDp;
            double d1 = map.invTransform( p1 );
            double d2 = map.invTransform( p2 );
//             const double center =
//                 scaleDiv.lowerBound() + scaleDiv.range() / 2;
//             const double width_2 = scaleDiv.range() / 2 * factor;
            plt->setAxisScale( axisId, d1, d2 );
