...
public:
    ...
    QPointer<QwtTextLabel> scaleIndexLabel[QwtPlot::axisCnt];
...
void QwtPlot::initPlot( const QwtText &title )
{
    ...
    QwtText text( title );
    text.setRenderFlags( Qt::AlignCenter | Qt::TextSingleLine );
    ...
    QwtText footer;
    footer.setRenderFlags( Qt::AlignCenter | Qt::TextSingleLine );
    d_data->footerLabel->setText( footer );

    //scaleIndex
    for (int i = 0; i < axisCnt; i++)
    {
        d_data->scaleIndexLabel[i] = new QwtTextLabel(this);
        d_data->scaleIndexLabel[i]->setObjectName("SimPlotScaleIndex");
    }
    // legend
    d_data->legend = NULL;
    ...
    {
        if (plotLayout()->fixedDimOfAxis(i) != 0)
        {
            axisWidget(i)->setLayoutFlag(QwtScaleWidget::ExtentFixed, true);
        }
        else
        {
            axisWidget(i)->setLayoutFlag(QwtScaleWidget::ExtentFixed, false);
        }
    }
}

QwtText QwtPlot::ScaleIndexLabel(int axisId) const
{
    return d_data->scaleIndexLabel[axisId]->text();
}

void QwtPlot::setScaleIndexLabel(int axisId, const QwtText& scale_index_text)
{
    d_data->scaleIndexLabel[axisId]->setText(scale_index_text);
}
...
void QwtPlot::setScaleIndexVisible( int axisId, bool visible )
{
    d_data->scaleIndexLabel[axisId]->setVisible(visible);
}

bool QwtPlot::scaleIndexVisible(int axisId)
{
    return d_data->scaleIndexLabel[axisId]->isVisible();
}
...
/*!
  \brief 根据坐标轴上对应刻度的值计算相应刻度指数值
  \param axisId 待计算的坐标轴
*/
void QwtPlot::updateScaleIndexText( int axisId )
{
    if ( !axisEnabled( axisId ) || axisScaleDraw(axisId)->getScaleDrawMode() != QwtScaleDraw::DrawScaleIndexMode )
    {
        setScaleIndexLabel( axisId, QwtText() );
        return;
    }
    double min_value = axisScaleDiv( axisId ).lowerBound();
    double max_value = axisScaleDiv( axisId ).upperBound();
    ...
    if (index_value != 0)
    {
        ...
        setScaleIndexLabel( axisId, index_text );
    }
    else
    {
        setScaleIndexLabel( axisId, QwtText() );
...
{   
    ...
    if ( !d_data->titleLabel->text().isEmpty() && titleRect.isValid() )
    {
    ...
    if ( !d_data->footerLabel->text().isEmpty() && footerRect.isValid() )
    {
    ...
    if ( d_data->legend )
    {
        if ( d_data->legend->isEmpty() || !legendRect.isValid() || plotLayout()->legendPosition() == QwtPlot::HideLegend)
        {
            d_data->legend->hide();
        }
        else
    ...
    if ( axisEnabled( axisId ) )
    {
        ...
        if ( axisId == yLeft || axisId == yRight )
        {
            ...
            double y = s->y() + s->startBorderDist() - d_data->canvas->y() 
                       + (plotLayout()->scaleVisible() ? s->scaleIndexSize().height() : 0);
        ...
        else
        {
            ...
            double w = s->width() - s->startBorderDist() - s->endBorderDist() 
                       - (plotLayout()->scaleVisible() ? s->scaleIndexSize().width() : 0);
...
void QwtPlot::insertLegend( QwtAbstractLegend *legend,
    QwtPlot::LegendLayout pos, double ratio )
{
    ...
    if ( legend != d_data->legend )
    {
        ...
        if ( d_data->legend )
        {
            ...
            connect(d_data->legend, SIGNAL(
                legendMoved(QPoint) ), this, SLOT(adjustLegendOffset(QPoint)));
...
/*!
  Set pos for float legend.
 */
void QwtPlot::adjustLegendOffset(QPoint pos_change)
{
    plotLayout()->adjustLegendOffset(pos_change.x(), pos_change.y());
    updateLayout();
}
